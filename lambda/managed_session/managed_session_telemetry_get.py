...
def managed_session_telemetry_get(managed_session_id):
    ...
    if vin is None:
        raise ArcimotoException(f'Unable to get vin for managed session {managed_session_id}')

    try:
        managed_session_start = managed_session.get('initialization', None)
        if managed_session_start is None:
            raise ArcimotoException('Invalid Managed Session initialization')
        ...
        raise ArcimotoException(f'Unable to process start timestamp: {e}')
    ...
    try:
        ...
        raise ArcimotoException(f'Unable to process end timestamp: {e}')
    ...
    try:
        ...
        raise ArcimotoException(f'Unable to setup telemetry point managed session period for influx requests: {e}')
...
lambda_handler = managed_session_telemetry_get
