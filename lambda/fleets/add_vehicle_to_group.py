...
def add_vehicle_to_group(vin, group_id):
    vehicle_instance = arcimoto.vehicle.Vehicle(vin)
    vehicle_instance.add_to_group(group_id)
    try:
        msg_part = f'Vehicle added to fleet id {group_id}'
        ...
        error_msg = f'Failure to create vehicle note for {vin} addition to vehicle group id {group_id}: - {e}'
...
lambda_handler = add_vehicle_to_group
