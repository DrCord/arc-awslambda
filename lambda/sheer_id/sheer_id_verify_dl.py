...
@arcimoto.user.require('sheer_id.verify.dl')
def sheer_id_verify_dl(first_name, last_name, email, drivers_license_number, state):
    global logger

    BASE_URL = 'https://services.sheerid.com/rest/v2/'
    program_id_secret = arcimoto.runtime.get_secret(f'sheer_id.program_id.{arcimoto.runtime.get_env()}')
    ...
    try:
        sheer_id_secret = arcimoto.runtime.get_secret('sheer_id.api')
        api_call_headers = {
            'Authorization': 'Bearer ' + str(sheer_id_secret.get('api_key')),
            'Content-Type': 'application/json'
        }
    except Exception as e:
        raise ArcimotoException(f'Unable to get create headers for request: {e}') from e
    ...
    try:
        ...
        raise ArcimotoException(f'Unable to complete POST request - error: {e}') from e
...
lambda_handler = sheer_id_verify_dl
