...
def users_user_mfa_sms_disable(username):
    global logger
    ...
    try:
        cognito_user_mfa_disable_sms_old(username)
        cognito_user_mfa_disable_sms(username)
...
def cognito_user_mfa_disable_sms(username):
    global logger, boto
    ...
    try:
        ...
        raise ArcimotoException('Unable to disable MFA using admin_set_user_mfa_preference for cognito user: {}'.format(e))


def cognito_user_mfa_disable_sms_old(username):
    '''
    ...
    try:
        ...
        raise ArcimotoException('Unable to disable MFA using admin_set_user_settings for cognito user: {}'.format(e))


lambda_handler = users_user_mfa_sms_disable
